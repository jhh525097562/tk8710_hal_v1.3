...
static int test_auto_downlink_tx_all(const uint8_t* userIndices, const uint8_t* txPowers, uint8_t userCount)
{
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        /* 设置自动发送用户数据和功率 */
        ret = TK8710SetTxUserDataWithPower(userIndex, txData, 26, txPower, 0);
        if (ret != TK8710_OK) {
...
int test_broadcast_tx(uint8_t counter)
{
    ...
    /* 使用新的广播发送API设置广播数据 */
    int ret = TK8710SetTxBrdDataWithPower(0, brdData, 26, 35, 0);  /* 广播用户0，功率35 */
    if (ret == TK8710_OK) {
...
static int test_manual_tx_setup(const uint8_t* userIndices, uint8_t userCount)
{
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        ret = TK8710SetTxUserDataWithPower(txUserIndex, txData, 26, 35, 0);  /* 使用连续索引，功率35 */
        if (ret != TK8710_OK) {
            printf("Failed to set TX user data for user[%d]: %d\n", txUserIndex, ret);
            return ret;
        }
