...
#include <stdint.h>
#include <stdbool.h>
#include "../inc/hal_api.h"
#include "../inc/phy/phy_api.h"
...
static int test_auto_downlink_tx_all(const uint8_t* userIndices, const uint8_t* txPowers, uint8_t userCount)
{
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        /* 设置自动发送下行2数据和功率 */
        ret = TK8710SetTxUserData(TK8710_DOWNLINK_2, userIndex, txData, 26, txPower, TK8710_DATA_TYPE_DED);
        if (ret != TK8710_OK) {
...
int test_broadcast_tx(uint8_t counter)
{
    ...
    /* 使用新的下行1发送API设置下行1数据 */
    int ret = TK8710SetTxUserData(TK8710_DOWNLINK_1, 0, brdData, 26, 35, TK8710_DATA_TYPE_BRD);  /* 下行1用户0，功率35 */
    if (ret == TK8710_OK) {
...
static int test_manual_tx_setup(const uint8_t* userIndices, uint8_t userCount)
{
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        ret = TK8710SetTxUserData(TK8710_DOWNLINK_2, txUserIndex, txData, 26, 35, TK8710_DATA_TYPE_DED);  /* 使用连续索引，功率35 */
        if (ret != TK8710_OK) {
            printf("Failed to set TX user data for user[%d]: %d\n", txUserIndex, ret);
            return ret;
        }
