 ...
 *        - TK8710_CFG_TYPE_FIX_INFO_LIST: 配置波束资源参数 (fixInfo_t)
 *        - TK8710_CFG_TYPE_DL_USER_POWER: 配置下行用户发射功率 (pwrCfg_t)
...
int TK8710SetConfig(TK8710ConfigType type, const void* params)
{
    ...
    switch (type) {
        ...
        case TK8710_CFG_TYPE_FIX_INFO_LIST:
        {
            /* 设置用户波束信息 (fixInfo_t) */
            const fixInfo_t* fixInfo = (const fixInfo_t*)params;
            uint8_t txBuf[640];  /* 最大128*5=640 bytes */
            uint16_t writeLen;
            int i;
            int ret;
            uint8_t brdUserNum = TK8710GetBrdUserNum();
            uint8_t dataUserNum = fixInfo->dataUserNum;
            RateModeParams rateModeParams;
            uint8_t maxUsers;
            s_init_13 userVal0;
            s_init_14 userVal1;
            s_init_15 userVal2;
            ...
            /* 获取当前速率模式的最大用户数 */
            ret = TK8710GetRateModeParams(TK8710GetRateMode(), &rateModeParams);
            if (ret != TK8710_OK) return ret;
            maxUsers = rateModeParams.maxUsers;
            
            /* 检查用户数是否超过最大值 */
            if (brdUserNum > TK8710_MAX_BRD_USERS) {
                brdUserNum = TK8710_MAX_BRD_USERS;
            }
            if (dataUserNum > maxUsers - brdUserNum) {
                dataUserNum = maxUsers - brdUserNum;
            }
            
            /* 1. 配置brd_user_val寄存器 (init_17, 0x4c) */
            /* 广播用户valid位: bit[brdUserNum-1:0] = 1 */
            brdUserVal.data = 0;
            brdUserVal.b.brd_user_val = (1 << brdUserNum) - 1;
            ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, MAC_BASE + offsetof(struct mac, init_17), brdUserVal.data);
            if (ret != TK8710_OK) return ret;
            
            /* 2. 配置user_val0-3寄存器 (init_13~16, 0x3c~0x48) */
            /* dataUserNum为0时，只配置广播，user_val全为0 */
            /* dataUserNum>0时，配置数据用户valid位 */
            userVal0.data = 0;
            userVal1.data = 0;
            userVal2.data = 0;
            userVal3.data = 0;
            ...
            if (dataUserNum > 0) {
                /* user_val0: bit[31:0] for user 0-31 */
                if (dataUserNum >= 32) {
                    userVal0.b.user_val0 = 0xFFFFFFFF;
                } else {
                    userVal0.b.user_val0 = (1UL << dataUserNum) - 1;
                }
                
                /* user_val1: bit[31:0] for user 32-63 */
                if (dataUserNum >= 64) {
                    userVal1.b.user_val1 = 0xFFFFFFFF;
                } else if (dataUserNum > 32) {
                    userVal1.b.user_val1 = (1UL << (dataUserNum - 32)) - 1;
                }
                
                /* user_val2: bit[31:0] for user 64-95 */
                if (dataUserNum >= 96) {
                    userVal2.b.user_val2 = 0xFFFFFFFF;
                } else if (dataUserNum > 64) {
                    userVal2.b.user_val2 = (1UL << (dataUserNum - 64)) - 1;
                }
                
                /* user_val3: bit[31:0] for user 96-127 */
                if (dataUserNum >= 128) {
                    userVal3.b.user_val3 = 0xFFFFFFFF;
                } else if (dataUserNum > 96) {
                    userVal3.b.user_val3 = (1UL << (dataUserNum - 96)) - 1;
                }
            }
            
            ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, MAC_BASE + offsetof(struct mac, init_13), userVal0.data);
            if (ret != TK8710_OK) return ret;
            ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, MAC_BASE + offsetof(struct mac, init_14), userVal1.data);
            if (ret != TK8710_OK) return ret;
            ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, MAC_BASE + offsetof(struct mac, init_15), userVal2.data);
            if (ret != TK8710_OK) return ret;
            ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, MAC_BASE + offsetof(struct mac, init_16), userVal3.data);
            if (ret != TK8710_OK) return ret;
            
            /* 3. 设置频点 (SetInfo type=0) */
            /* freqList已按广播、数据用户顺序排列 */
            if (fixInfo->freqList != NULL) {
                uint8_t totalUsers = brdUserNum + dataUserNum;
                writeLen = totalUsers * 4;
                for (i = 0; i < totalUsers; i++) {
                    txBuf[i*4]   = (uint8_t)(fixInfo->freqList[i] >> 24);
                    txBuf[i*4+1] = (uint8_t)(fixInfo->freqList[i] >> 16);
                    txBuf[i*4+2] = (uint8_t)(fixInfo->freqList[i] >> 8);
                    txBuf[i*4+3] = (uint8_t)(fixInfo->freqList[i]);
                }
                ret = TK8710SpiSetInfo(TK8710_GET_INFO_FREQ, txBuf, writeLen);
                if (ret != 0) return TK8710_ERR;
            }
            
            /* 4. 设置AH数据 (SetInfo type=1) */
            if (fixInfo->ahList != NULL) {
                uint8_t totalUsers = brdUserNum + dataUserNum;
                writeLen = totalUsers * 40;  /* 每个用户40字节 = 320bit (8天线) */
                for (i = 0; i < totalUsers; i++) {
                    /* 设置所有8天线的AH数据 */
                    /* ahList存储格式: ahList[i*16 + ant*2]=I, ahList[i*16 + ant*2+1]=Q */
                    for (int ant = 0; ant < 8; ant++) {
                        uint64_t ah40 = ((uint64_t)(fixInfo->ahList[i*16 + ant*2] & 0xFFFFF) << 20) |
                                        ((uint64_t)(fixInfo->ahList[i*16 + ant*2 + 1] & 0xFFFFF));
                        txBuf[i*40 + ant*5]   = (uint8_t)(ah40 >> 32);
                        txBuf[i*40 + ant*5 + 1] = (uint8_t)(ah40 >> 24);
                        txBuf[i*40 + ant*5 + 2] = (uint8_t)(ah40 >> 16);
                        txBuf[i*40 + ant*5 + 3] = (uint8_t)(ah40 >> 8);
                        txBuf[i*40 + ant*5 + 4] = (uint8_t)(ah40);
                    }
                }
                ret = TK8710SpiSetInfo(TK8710_GET_INFO_AH, txBuf, writeLen);
                if (ret != 0) return TK8710_ERR;
            }
            
            /* 5. 设置Pilot功率 (SetInfo type=3) */
            /* 按最大用户数配置，实际用户数小于最大用户数时，剩余pilotpow填0 */
            if (fixInfo->pilotPwr != NULL) {
                uint8_t actualUsers = brdUserNum + dataUserNum;
                writeLen = maxUsers * 5;  /* 按速率模式最大用户数写入 */
                for (i = 0; i < maxUsers; i++) {
                    uint64_t pow40;
                    if (i < actualUsers) {
                        /* 有效用户: 使用输入数据 */
                        pow40 = ((uint64_t)(fixInfo->pilotPwr[i*2] & 0xF) << 32) |
                                ((uint64_t)fixInfo->pilotPwr[i*2+1]);
                    } else {
                        /* 剩余用户: 填0 */
                        pow40 = 0;
                    }
                    txBuf[i*5]   = (uint8_t)(pow40 >> 32);
                    txBuf[i*5+1] = (uint8_t)(pow40 >> 24);
                    txBuf[i*5+2] = (uint8_t)(pow40 >> 16);
                    txBuf[i*5+3] = (uint8_t)(pow40 >> 8);
                    txBuf[i*5+4] = (uint8_t)(pow40);
                }
                ret = TK8710SpiSetInfo(TK8710_GET_INFO_PILOT_POW, txBuf, writeLen);
                if (ret != 0) return TK8710_ERR;
            }
            
            /* 6. 设置Anoise数据 (SetInfo type=2), 8天线 */
            /* 每天线anoise为16bit, u16.11格式 */
            if (fixInfo->anoise != NULL) {
                writeLen = TK8710_MAX_ANTENNAS * 2;  /* 每天线16bit = 2 bytes */
                for (i = 0; i < TK8710_MAX_ANTENNAS; i++) {
                    txBuf[i*2]   = (uint8_t)(fixInfo->anoise[i] >> 8);
                    txBuf[i*2+1] = (uint8_t)(fixInfo->anoise[i]);
                }
                ret = TK8710SpiSetInfo(TK8710_GET_INFO_ANOISE, txBuf, writeLen);
                if (ret != 0) return TK8710_ERR;
            }
        ...
        case TK8710_CFG_TYPE_DL_USER_POWER:
        {
            /* 配置mac寄存器tx_pow_ctrl (0x70) */
            /* [15:8] UserIndex: 0~127数据用户, 128~143广播用户 */
            /* [7:0] power: tx power value */
            pwrCfg_t* pwrCfg = (pwrCfg_t*)params;
            s_tx_pow_ctrl tx_pow_ctrl;
            size_t i;
            int ret;
            
            if (pwrCfg == NULL || pwrCfg->tx_power == NULL) {
                return TK8710_ERR;
            }
            
            tx_pow_ctrl.data = 0;
            for (i = 0; i < pwrCfg->count; i++) {
                tx_pow_ctrl.b.UserIndex = pwrCfg->start_index + i;
                tx_pow_ctrl.b.power = pwrCfg->tx_power[i] & 0xFF;
                ret = TK8710WriteReg(TK8710_REG_TYPE_GLOBAL, 
 ...
 *        - TK8710_CFG_TYPE_FIX_INFO_LIST: 获取用户波束信息 (fixInfo_t)
 *        - TK8710_CFG_TYPE_DL_USER_POWER: 获取下行用户发射功率 (pwrCfg_t)
...
int TK8710GetConfig(TK8710ConfigType type, void* params)
{
    ...
    switch (type) {
        ...
        {
            /* 获取用户波束信息 (fixInfo_t) */
            fixInfo_t* fixInfo = (fixInfo_t*)params;
            RateModeParams rateModeParams;
            uint8_t rateMode;
            uint8_t maxUsers;
            uint16_t readLen;
            uint8_t rxBuf[640];  /* 最大128*5=640 bytes (AH数据) */
            int i;
            
            /* 获取当前速率模式和最大用户数 */
            rateMode = TK8710GetRateMode();
            ret = TK8710GetRateModeParams(rateMode, &rateModeParams);
            if (ret != TK8710_OK) return ret;
            
            maxUsers = rateModeParams.maxUsers;
            fixInfo->dataUserNum = maxUsers;
            
            /* 1. 获取用户频点 (GetInfo type=0) */
            if (fixInfo->freqList != NULL) {
                readLen = maxUsers * 4;
                ...
                for (i = 0; i < maxUsers; i++) {
                    fixInfo->freqList[i] = ((uint32_t)rxBuf[i*4] << 24) |
            ...
            /* 2. 获取AH数据 (GetInfo type=1) */
            /* 每个用户AH为40bit × 8天线 = 320bit = 40字节, {20bit I, 20bit Q}每天线, s20.13格式 */
            if (fixInfo->ahList != NULL) {
                readLen = maxUsers * 40;  /* 每个用户40字节 = 320bit */
                ret = TK8710SpiGetInfo(TK8710_GET_INFO_AH, rxBuf, readLen);
                if (ret != 0) return TK8710_ERR;
                ...
                for (i = 0; i < maxUsers; i++) {
                    ...
                    /* ahList存储格式: ahList[i*16 + ant*2]=I, ahList[i*16 + ant*2+1]=Q */
                    for (int ant = 0; ant < 8; ant++) {
                        uint64_t ah40 = ((uint64_t)rxBuf[i*40 + ant*5] << 32) |
                                        ((uint64_t)rxBuf[i*40 + ant*5 + 1] << 24) |
                                        ((uint64_t)rxBuf[i*40 + ant*5 + 2] << 16) |
                                        ((uint64_t)rxBuf[i*40 + ant*5 + 3] << 8) |
                                        ((uint64_t)rxBuf[i*40 + ant*5 + 4]);
                        
                        /* 保存每个天线的AH数据到ahList */
                        fixInfo->ahList[i*16 + ant*2] = (uint32_t)((ah40 >> 20) & 0xFFFFF);   /* I: 20bit */
                        fixInfo->ahList[i*16 + ant*2 + 1] = (uint32_t)(ah40 & 0xFFFFF);      /* Q: 20bit */
                    }
                }
            }
            
            /* 3. 获取Pilot功率 (GetInfo type=3) */
            /* 每个用户pilot pow为40bit {4bit 0, 36bit pow}, u36.30格式 */
            if (fixInfo->pilotPwr != NULL) {
                readLen = maxUsers * 5;  /* 每个用户40bit = 5 bytes */
                ret = TK8710SpiGetInfo(TK8710_GET_INFO_PILOT_POW, rxBuf, readLen);
                if (ret != 0) return TK8710_ERR;
                
                for (i = 0; i < maxUsers; i++) {
                    /* 40bit数据: [39:36]=0, [35:0]=pow(36bit) */
                    /* pilotPwr存储两个元素: pilotPwr[i*2]=高4bit, pilotPwr[i*2+1]=低32bit */
                    uint64_t pow40 = ((uint64_t)rxBuf[i*5] << 32) |
                                     ((uint64_t)rxBuf[i*5+1] << 24) |
                                     ((uint64_t)rxBuf[i*5+2] << 16) |
                                     ((uint64_t)rxBuf[i*5+3] << 8) |
                                     ((uint64_t)rxBuf[i*5+4]);
                    fixInfo->pilotPwr[i*2] = (uint32_t)((pow40 >> 32) & 0xF);      /* 高4bit */
                    fixInfo->pilotPwr[i*2+1] = (uint32_t)(pow40 & 0xFFFFFFFF);     /* 低32bit */
                }
            }
            
            /* 4. 获取Anoise数据 (GetInfo type=2), 8天线 */
            /* 每天线anoise为16bit, u16.11格式 */
            if (fixInfo->anoise != NULL) {
                readLen = TK8710_MAX_ANTENNAS * 2;  /* 每天线16bit = 2 bytes */
                ret = TK8710SpiGetInfo(TK8710_GET_INFO_ANOISE, rxBuf, readLen);
                if (ret != 0) return TK8710_ERR;
                
                for (i = 0; i < TK8710_MAX_ANTENNAS; i++) {
                    fixInfo->anoise[i] = ((uint32_t)rxBuf[i*2] << 8) |
                                         ((uint32_t)rxBuf[i*2+1]);
                }
            }
