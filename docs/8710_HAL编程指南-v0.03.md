...
| 修订时间   | 修订版本 | 修订描述                                                    | 修订人 |
| ---------- | -------- | ----------------------------------------------------------- | ------ |
| 2026/03/06 | V0.003   | - 调整部分内容描述<br />- 新增第5章节TK8710 PHY API接口定义 | 黄家豪 |
...
TK8710 HAL（Hardware Abstraction Layer，硬件抽象层）为应用开发者提供了一套简洁、统一的编程接口，屏蔽了底层硬件的复杂性。HAL内部采用分层架构设计，由**TRM层**和**PHY driver层**两个核心模块组成。

![image-20260306142243489](D:/Windsurf_workspace/RK3506/tk8710_hal_v1.3/docs/assets/image-20260306142243489.png)
...
| **PHY driver层** | 硬件驱动层 | 硬件抽象 | 负责SPI通信、中断处理、寄存器操作等硬件交互    |

#### 2.1.2 TRM层定位

TRM（Transceiver Resource Manager，收发资源管理器）是HAL的业务逻辑核心，位于HAL API和PHY driver层之间：

- **向上**: 接收HAL API的调用请求，处理业务逻辑
- **向下**: 调用PHY driver层接口完成硬件操作

#### 2.1.3 PHY driver层定位

PHY driver层是HAL的硬件抽象核心，直接与TK8710芯片交互：

- **向上**: 为TRM层提供数据收发、配置等接口
- **向下**: 通过SPI接口与TK8710芯片通信

> **注**: PHY driver层API接口见后续第5章节。

**架构特点**:

- **接口简洁**: 仅需6个核心API即可完成所有操作
- **分层设计**: TRM层处理业务逻辑，PHY driver层处理硬件交互
...
| **状态监控**   | 运行状态查询、统计信息获取                          |
| **系统控制**   | 芯片启动、复位、调试（寄存器读写、TRM和PHY driver） |
...
sequenceDiagram
    ...
    participant DRV as PHY driver层
    participant HW as TK8710芯片
  
    Note over APP,HW: 初始化阶段
    APP->>HAL: 调用初始化接口
    HAL->>TRM: 初始化TRM模块
    TRM->>DRV: 初始化PHY driver模块
    DRV->>HW: 芯片硬件初始化
    HW-->>DRV: 初始化完成
    DRV-->>TRM: PHY driver就绪
    TRM-->>HAL: TRM就绪
    HAL-->>APP: 初始化完成
...
- **数据发送调度**: 管理发送队列，按照时隙结构调度数据发送
- **超帧管理**: 维护TDD帧同步，管理BCN发送和接收
- **波束管理**: 维护用户波束信息，支持广播和专用波束
- **资源调度**: 分配和管理时隙资源等

#### 2.4.3 PHY driver层工作机制

PHY driver层作为硬件抽象核心，主要负责：

- **SPI通信**: 处理与TK8710芯片的SPI数据传输
- **中断处理**: 响应芯片中断，提取接收数据和状态信息
- **寄存器操作**: 读写芯片寄存器，完成硬件配置
- **硬件状态监控**: 监控芯片工作状态，上报异常情况

#### 2.4.4 异步处理机制

HAL采用异步处理机制提高系统效率：

- **发送异步**: `TK8710HAL_sendData()`立即返回，实际发送在后台进行
- **接收异步**: 通过中断触发，回调函数通知应用层
- **状态查询**: 同步接口，实时获取系统状态

这种异步机制确保了应用层不会被硬件操作阻塞，提高了系统的响应性能。
...
| API函数                 | 功能描述  | 参数             | 返回值        |
| ----------------------- | --------- | ---------------- | ------------- |
| `TK8710HAL_init`      | HAL初始化 | 配置结构体指针   | 成功/失败状态 |
| `TK8710HAL_config`    | HAL配置   | 时隙配置指针     | 成功/失败状态 |
| `TK8710HAL_start`     | 启动HAL   | 无               | 成功/失败状态 |
| `TK8710HAL_reset`     | 复位HAL   | 无               | 成功/失败状态 |
| `TK8710HAL_sendData`  | 发送数据  | 完整的发送参数   | 成功/失败状态 |
| `TK8710HAL_getStatus` | 获取状态  | 统计信息输出指针 | 成功/失败状态 |
| `TK8710HAL_debug`     | 调试接口  | 调试类型和参数   | 成功/失败状态 |
| TK8710HAL_cali          | 校准接口  | 自定义           | 成功/失败     |
...
#### 3.2.1 TK8710HAL_init - 初始化

```c
TK8710_HALError TK8710HAL_init(const TK8710_HALInitConfig* config);
```
...
| 返回值                       | 说明       |
| ---------------------------- | ---------- |
| TK8710_TK8710HAL_OK          | 初始化成功 |
| TK8710_TK8710HAL_ERROR_PARAM | 参数错误   |
| TK8710_TK8710HAL_ERROR_INIT  | 初始化失败 |

**使用示例**:

```c
/* 使用默认配置初始化 */
TK8710_HALError ret = TK8710HAL_init(NULL);
if (ret != TK8710_TK8710HAL_OK) {
    printf("HAL初始化失败: %d\n", ret);
    return -1;
}
...
#### 3.2.2 TK8710HAL_config - 配置

```c
TK8710_HALError TK8710HAL_config(const slotCfg_t* slotConfig, uint32_t N_sync);
```
...
| 返回值                        | 说明     |
| ----------------------------- | -------- |
| TK8710_TK8710HAL_OK           | 配置成功 |
| TK8710_TK8710HAL_ERROR_CONFIG | 配置失败 |

**使用示例**:

```c
/* 使用当前配置 */
TK8710_HALError ret = TK8710HAL_config(NULL);

/* 使用自定义时隙配置 */

ret = TK8710HAL_config(&slotConfig);
if (ret != TK8710_TK8710HAL_OK) {
    printf("HAL配置失败: %d\n", ret);
}
```
...
#### 3.2.3 TK8710HAL_start - 启动

```c
TK8710_HALError TK8710HAL_start(void);
```
...
| 返回值                       | 说明     |
| ---------------------------- | -------- |
| TK8710_TK8710HAL_OK          | 启动成功 |
| TK8710_TK8710HAL_ERROR_START | 启动失败 |
...
```c
TK8710_HALError ret = TK8710HAL_start();
if (ret != TK8710_TK8710HAL_OK) {
    printf("HAL启动失败: %d\n", ret);
    return -1;
}
...
#### 3.2.4 TK8710HAL_reset - 复位

```c
TK8710_HALError TK8710HAL_reset(void);
```
...
| 返回值                       | 说明     |
| ---------------------------- | -------- |
| TK8710_TK8710HAL_OK          | 复位成功 |
| TK8710_TK8710HAL_ERROR_RESET | 复位失败 |

**说明**:

- 完全复位状态机和寄存器
- 清理系统资源
- 复位后需要重新调用 `TK8710HAL_init()` 才能使用

**使用示例**:

```c
TK8710_HALError ret = TK8710HAL_reset();
if (ret != TK8710_TK8710HAL_OK) {
    printf("HAL复位失败: %d\n", ret);
    return -1;
}
...
#### 3.2.5 TK8710HAL_sendData - 发送数据

```c
TK8710_HALError TK8710HAL_sendData(
...
| 返回值                      | 说明                       |
| --------------------------- | -------------------------- |
| TK8710_TK8710HAL_OK         | 发送成功（已加入发送队列） |
| TK8710_TK8710HAL_ERROR_SEND | 发送失败                   |
...
TK8710_HALError ret = TK8710HAL_sendData(
...
#### 3.2.6 TK8710HAL_getStatus - 获取状态

```c
TK8710_HALError TK8710HAL_getStatus(TRM_Stats* stats);
```
...
| 返回值                        | 说明                    |
| ----------------------------- | ----------------------- |
| TK8710_TK8710HAL_OK           | 获取成功                |
| TK8710_TK8710HAL_ERROR_PARAM  | 参数错误（stats为NULL） |
| TK8710_TK8710HAL_ERROR_STATUS | 状态查询失败            |
...
```c
TRM_Stats stats;
TK8710_HALError ret = TK8710HAL_getStatus(&stats);
if (ret == TK8710_TK8710HAL_OK) {
    printf("=== HAL状态信息 ===\n");
    printf("运行状态: %d\n", stats.state);
...
#### 3.2.7 TK8710HAL_debug - 调试接口

```c
TK8710_HALError TK8710HAL_debug(uint32_t type, void* para1, void* para2);
```
...
| 返回值                       | 说明                       |
| ---------------------------- | -------------------------- |
| TK8710_TK8710HAL_OK          | 调试操作成功               |
| TK8710_TK8710HAL_ERROR_PARAM | 参数错误或不支持的调试类型 |
...
```c
/* 系统调试 */
TK8710_HALError ret = TK8710HAL_debug(0, NULL，NULL);

/* 读/写取寄存器 */
ret = TK8710HAL_debug(1, regaddr，regData);
```
...
sequenceDiagram
    ...
    Note over APP,HW: 初始化阶段
    APP->>HAL: TK8710HAL_init()
    HAL->>HW: 芯片初始化/RF配置
    HW-->>HAL: 初始化完成
    HAL-->>APP: 返回成功
  
    Note over APP,HW: 配置阶段
    APP->>HAL: TK8710HAL_config()
    HAL->>HW: 时隙配置
    HW-->>HAL: 配置完成
    HAL-->>APP: 返回成功
  
    Note over APP,HW: 启动阶段
    APP->>HAL: TK8710HAL_start()
    HAL->>HW: 启动工作
    HW-->>HAL: 启动完成
    HAL-->>APP: 返回成功
  
    Note over APP,HW: 数据收发阶段
    APP->>HAL: TK8710HAL_sendData()
    HAL->>HW: 发送数据
    HW-->>HAL: 发送完成
    HAL-->>APP: 回调通知
...
#### 3.3.1 TK8710_Init - 芯片初始化

```c
int TK8710_Init(const ChiprfConfig* rfConfig, const ChipConfig* chipConfig);
```
...
**返回值**: TK8710_OK成功，其他失败

**说明**: 完成芯片硬件初始化，包括SPI接口、时钟、基本寄存器配置等

#### 3.3.2 TK8710_Config - 芯片配置

```c
int TK8710_Config(const slotCfg_t* slotConfig);
```
...
#### 3.3.4 TK8710_SendData - 发送数据

```c
int TK8710SetTxData(TK8710DownlinkType downlinkType, uint8_t userId_brdIndex, 
...
#### 3.3.5 TK8710_ReadReg/TK8710_WriteReg - 寄存器读写

```c
int TK8710_ReadReg(uint32_t regAddr, uint32_t* regData);
int TK8710_WriteReg(uint32_t regAddr, uint32_t regData);
```
...
```c
#include "TK8710_TK8710HAL_api.h"
#include <stdio.h>
...
int main(void)
{
    ...
    ret = TK8710HAL_sendData(TK8710_DOWNLINK_A, 0, data, sizeof(data), 35, 0, 0, TK8710_DATA_TYPE_BRD);
    if (ret == TK8710_TK8710HAL_OK) {
        printf("数据发送成功\n");
    }
...
TK8710_HALError ret = TK8710HAL_init(&config);
```
...
TK8710_HALError ret = TK8710HAL_config(&slotConfig);
```
...
```c
/* 系统调试 */
TK8710HAL_debug(0, NULL);

/* 硬件调试 */
TK8710HAL_debug(1, NULL);

/* 通信调试 */
TK8710HAL_debug(2, NULL);

/* 中断调试 */
TK8710HAL_debug(3, NULL);
```
...
int main(void)
{
    ...
    ret = TK8710HAL_config(NULL);  /* 使用默认时隙配置 */
    if (ret != TK8710_TK8710HAL_OK) {
...
| 错误码                        | 数值 | 描述         | 可能原因                | 处理建议                 |
| ----------------------------- | ---- | ------------ | ----------------------- | ------------------------ |
| TK8710_TK8710HAL_OK           | 0    | 操作成功     | -                       | -                        |
| TK8710_TK8710HAL_ERROR_PARAM  | -1   | 参数错误     | 传入NULL指针或无效参数  | 检查参数有效性           |
| TK8710_TK8710HAL_ERROR_INIT   | -2   | 初始化失败   | SPI通信失败、芯片无响应 | 检查硬件连接和SPI配置    |
| TK8710_TK8710HAL_ERROR_CONFIG | -3   | 配置失败     | 配置参数无效            | 检查时隙配置参数         |
| TK8710_TK8710HAL_ERROR_START  | -4   | 启动失败     | 芯片状态异常            | 尝试复位后重新初始化     |
| TK8710_TK8710HAL_ERROR_SEND   | -5   | 发送失败     | 发送队列满、参数无效    | 检查队列状态和参数       |
| TK8710_TK8710HAL_ERROR_STATUS | -6   | 状态查询失败 | 系统未初始化            | 确保已调用TK8710HAL_init |
| TK8710_TK8710HAL_ERROR_RESET  | -7   | 复位失败     | 硬件异常                | 检查硬件状态             |

### 5.2 常见问题FAQ

#### Q1: TK8710HAL_init返回ERROR_INIT，如何排查？
...
2. **TK8710_TK8710HAL_API.md** - HAL API详细接口文档
