...
2. **时隙长度计算**:
   - BCN时隙 = 固定值（基于g_bcnSlotLen表）
   - 广播时隙 = 1024us + g_brdBaseBody[mode] × (brdBlockNum × 2 + 1) + g_brdBaseGap[mode]
   - 上行时隙 = 1024us + g_ulBaseBody[mode] × (ulBlockNum × 2 + 1) + g_ulBaseGap[mode]
   - 下行时隙 = 1024us + g_dlBaseBody[mode] × (dlBlockNum × 2 + 1) + g_dlBaseGap[mode]
3. **帧周期优化**（新算法）:
   - 遍历M=1-10秒和N=1-64的所有组合
   - 严格满足 `framePeriod × frameCount = M × 1,000,000 us`
   - N必须能整除totalUs（M秒的总微秒数）
   - 优先选择增加间隔最小的解，且N必须是superFrameNum的整数倍
   - 确保新帧周期 ≥ 原始帧周期
   - 如果gap为0，直接选择该最优解
...
- **M值范围**: 支持1-10秒范围内的任意M值
- **N值约束**: N值范围1-64，必须能整除M秒的总微秒数
- **超帧约束**: N必须是superFrameNum的整数倍
- **高效搜索**: 通过整除检查和早期退出机制快速找到最优解
- **兜底机制**: 如果找不到合适解，使用原始帧周期

**支持的速率模式**:

| 模式 | 系统带宽 | 用户数 | BCN时隙(us) | slot1（广播）主体 | slot2上行主体 | slot3下行主体 | slot1（广播）间隔 | slot2上行间隔 | slot3下行间隔 |
| ---- | -------- | ------ | ----------- | ----------------- | ------------- | ------------- | ---------------- | ------------- | ------------- |
| 5    | 62.5KHz  | 128    | 69583       | 131072            | 131072        | 131072        | 21492           | 21492         | 21492         |
| 6    | 125KHz   | 128    | 36340       | 65536             | 65536         | 65536         | 19728           | 19728         | 19728         |
| 7    | 250KHz   | 128    | 19129       | 32768             | 32768         | 32768         | 12000           | 12000         | 12000         |
| 8    | 500KHz   | 128    | 10732       | 16384             | 16384         | 16384         | 5600            | 5600          | 5600          |
| 9    | 500KHz   | 64     | 6510        | 8192              | 8192          | 8192          | 2800            | 2800          | 2800          |
| 10   | 500KHz   | 32     | 6510        | 4096              | 4096          | 4096          | 1400            | 1400          | 1400          |
