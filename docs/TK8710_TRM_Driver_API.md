...
```c
int TRM_SetTxData(TK8710DownlinkType downlinkType, uint32_t userId_brdIndex, const uint8_t* data, uint16_t len, uint8_t txPower, uint32_t frameNo, uint8_t targetRateMode, uint8_t dataType);
```
...
```c
// 发送广播数据
int ret = TRM_SetTxData(TK8710_DOWNLINK_1, 0, brdData, sizeof(brdData), 20, 0, 0, TK8710_DATA_TYPE_BRD);

// 发送用户数据
ret = TRM_SetTxData(TK8710_DOWNLINK_2, 0x12345678, userData, sizeof(userData), 25, frameNo, 0, TK8710_DATA_TYPE_DED);
```
...
ret = TRM_SetTxData(TK8710_DOWNLINK_2, 0x12345678, testData, sizeof(testData), 20, currentFrame + 1, 0, TK8710_DATA_TYPE_DED);

// 多速率模式发送(使用速率模式匹配)
ret = TRM_SetTxData(TK8710_DOWNLINK_2, 0x12345678, testData, sizeof(testData), 20, currentFrame + 1, TK8710_RATE_MODE_5, TK8710_DATA_TYPE_DED);
```
