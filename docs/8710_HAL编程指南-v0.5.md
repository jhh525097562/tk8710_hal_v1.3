...
| 修订时间   | 修订版本 | 修订描述                                                    | 修订人 |
| ---------- | -------- | ----------------------------------------------------------- | ------ |
| 2026/03/11 | v0.5     | - 调整部分内容<br />- 调整PHY API命名                       | 黄家豪 |
...
HAL内部包含了TRM和PHY Driver模块，主要提供以下核心功能：

| 功能模块             | 说明                                                | 模块                                            |
| -------------------- | --------------------------------------------------- | --------------------------------------------------- |
| **初始化管理** | 芯片初始化、射频配置、日志系统配置                  | PHY Driver        |
| **时隙配置**   | TDD帧结构配置、速率模式设置、天线配置、超帧配置     | TRM  |
| **数据接收/发送** | 广播数据发送、用户数据接收/发送            | TRM                   |
| **状态监控**   | 运行状态查询、统计信息获取                          | TRM、PHY Driver            |
| **系统控制**   | 芯片启动、复位、调试（寄存器读写、PHY driver） | TRM、PHY Driver |
...
HAL的整体工作流程可以分为初始化、配置、运行三个主要阶段：（HAL的数据接收、发送、超帧管理机制见附录中5.3章节）
...
| TK8710PhyRegisterCallbacks（TK8710PhySetCallbacks） | 注册中断回调函数                   |

#### 3.2.1 TK8710PhyInit - 芯片初始化

```c
int TK8710PhyInit(const ChiprfConfig* rfConfig, const ChipConfig* chipConfig);
```
...
#### 3.2.2 TK8710PhyConfig - 芯片配置

```c
int TK8710PhyConfig(const slotCfg_t* slotConfig);
```
...
**说明**: 根据复位类型执行芯片复位操作，可选择仅复位状态机或完全复位包括寄存器

#### 3.2.4 TK8710PhyStart - 启动芯片
...
- `workType`: 工作类型：0=Slave（从模式），1=Master（主模式）
- `workMode`: 工作模式：1=连续模式，2=单次模式
...
**说明**: 启动芯片进入指定的收发工作状态，支持主从模式选择和连续/单次工作模式

#### 3.2.5 TK8710PhySendData - 发送数据

```c
int TK8710PhySendData(TK8710DownlinkType downlinkType, uint8_t userId_brdIndex, 
...
#### 3.2.8 TK8710PhyRegisterCallbacks - 注册回调函数

```c
void TK8710PhyRegisterCallbacks(const TK8710DriverCallbacks* callbacks);
```
...
### 3.3 TK8710 HAL API详细说明

| 层级             | 模块       | 定位     | 说明                                           |
| ---------------- | ---------- | -------- | ---------------------------------------------- |
| **HAL API**      | 统一接口层 | 对外接口 | 提供简洁的API供MAC调用                         |
| **TRM层**        | 收发管理层 | 业务逻辑 | 负责数据发送调度、超帧管理、波束管理等业务逻辑 |
| **PHY driver层** | 硬件驱动层 | 硬件抽象 | 负责SPI通信、中断处理、寄存器操作等硬件交互    |

![image-20260311115423503](D:/Windsurf_workspace/RK3506/TK8710_hal_v1.3/docs/assets/image-20260311115423503.png)
...
| TK8710HalSetRxCallback | 设置接收数据回调                    |
| TK8710HalSetTxCallback | 设置发送完成回调                    |

#### 3.3.1 TK8710HalInit - 初始化
...
#### 3.3.8 TK8710HalSetRxCallback - 接收回调

```c
typedef void (*TK8710HalSetRxCallback  )(const RxDataList* rxDataList);
```
...
#### 3.3.9 TK8710HalSetTxCallback - 发送完成回调

```c
typedef void (*TK8710HalSetTxCallback )(const TxCompleteResult* txResult);
```
...
### 5.3TK8710 HAL主要工作机制说明

#### 5.3.1 TK8710 HAL数据接收流程
...
sequenceDiagram
    ...
    TRM->>APP: 数据接收回调(用户数据列表)
```

#### 5.3.2 TK8710 HAL数据发送流程
...
#### 5.3.3 TK8710 HAL超帧管理

HAL中的TRM模块负责管理超帧序号，在时间事件驱动模式下工作。

**超帧配置**:

MAC层通过 `TK8710HALConfig`中参数配置超帧大小（帧数），TRM根据配置维护超帧计数。
...
TK8710启动后，TRM在PHY driver时间事件驱动下工作。PHY driver层通过时隙结束回调（`OnSlotEnd`）通知TRM每个slot的结束事件。
...
sequenceDiagram
    ...
    alt 无Payload广播（网关自主管理）
        ...
        MAC->>GW: 下发广播帧（含BroadcastPayload）
