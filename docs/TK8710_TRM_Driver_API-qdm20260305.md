...
#### `Gateway_SetConfig` （qdm 帧配置，函数内存下消息体，并设置标志位，退出函数；主程序之后检查标志位，调用TRM时隙计算器，获得时隙配置for driver，获得N_sync for硬件控制，然后重启配置driver和硬件和TRM，init）（qdm 增加API支持上述操作，在示例流程中建议如何实现）（hjh 帧配置，函数内存下消息体，决定是否重启，如果需要重启，init）（qdm 找网关软件实现的人一起讨论，确定按哪种方案做）
