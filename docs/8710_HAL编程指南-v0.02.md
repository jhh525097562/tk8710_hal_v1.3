...
![9IOoNXAAAABklEQVQDAN0F3nQsRJV1AAAAAElFTkSuQmCC](D:/Windsurf_workspace/RK3506/tk8710_hal_v1.3/docs/assets/image001(03-06-10-30-30).png)
...
> **注**: PHY driver层的详细功能和机制说明、PHY driver API接口见后续《HAL概要设计》章节。
...
- **发送异步**: `HAL_sendData()`立即返回，实际发送在后台进行
...
本节从概要设计角度详细说明HAL内部TRM层和PHY driver层的功能定义、工作机制及层间接口设计。
...
| **分层解耦** | TRM层与PHY driver层职责分离，便于独立开发和维护 |
| **高效通信** | 优化数据收发流程，减少延迟和资源占用        |
| **可扩展性** | 支持不同硬件平台和RF配置的适配              |

**设计原则**:

- **分层架构**: TRM层处理业务逻辑，PHY driver层处理硬件交互
...
8710启动后，TRM在PHY driver时间事件驱动下工作。PHY driver层通过时隙结束回调（`OnSlotEnd`）通知TRM每个slot的结束事件。
...
PHY driver层是HAL的硬件抽象核心，负责与TK8710芯片的直接交互。
...
PHY driver通过SPI命令与TK8710芯片交互，主要命令类型如下：
...
PHY driver处理TK8710产生的中断事件，完成数据发送、接收、BCN和广播天线的轮流发送等。
...
PHY driver将寄存器读写操作封装为数据发送接口和接收数据存储：
...
TRM层与PHY driver层之间通过定义良好的接口进行交互。

#### 5.4.1 PHY driver向TRM提供的接口
...
#### 5.4.2 TRM向PHY driver注册的回调

| 回调           | 触发时机   | 说明                          |
| -------------- | ---------- | ----------------------------- |
| `OnRxData`     | 接收到数据 | PHY driver将接收数据传递给TRM |
| `OnTxComplete` | 发送完成   | PHY driver通知TRM发送结果     |
| `OnSlotEnd`    | 时隙结束   | PHY driver通知TRM时隙切换     |
| `OnError`      | 发生错误   | PHY driver通知TRM错误事件     |
...
flowchart TB
    subgraph 下行数据流
        ...
        TRM1 -->|TK8710_SendData| DRV1[PHY driver层]
        DRV1 -->|SPI写| HW1[TK8710]
    end
  
    subgraph 上行数据流
        HW2[TK8710] -->|中断| DRV2[PHY driver层]
