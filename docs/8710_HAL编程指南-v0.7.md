...
TK8710HalSlotConfig slotconfig = {
    /* 自定义配置参数 */
};
TK8710HalError ret = TK8710HalInit(&initconfig,&slotconfig);
if (ret != TK8710HAL_OK) {
    printf("HAL初始化失败: %d\n", ret);
    return -1;
}
...
int main(void)
{
    ...
    ret = TK8710HalInit(&initConfig,&slotConfig);
    if (ret != TK8710HAL_OK) {
