...
#### `TK8710SetDownlink1DataWithPower` ( qdm 1/2 or A/B)
...
- `power`: 发射功率 (0-31)
- `dataType`: 数据类型 （qdm：broadcast/dedicated 波束类型 TK8710_DATA_TYPE_BRD TK8710_DATA_TYPE_DED）
...
```c
int TK8710SetTxUserInfo(uint8_t userIndex, uint32_t freq, const uint8_t* ahData, uint8_t pilotPower);  （qdm rx）
```
...
#### `TK8710GetRxData`  （qdm TK8710GetRxUserData）
...
- `userIndex`: 用户索引 (0-127)
- `freq`: 输出频率指针 (Hz) (qdm: freqInfo，不能解读)
...
#### `TK8710GetSignalInfo` （qdm TK8710GetRxUserSignalQuality）
...
```c
typedef void (*TK8710RxDataCallback)(TK8710IrqResult* irqResult);  （qdm irqInfo）
...
申请（只申请有效的user的，用userIndex索引）、停止申请、释放
...
## TRM API接口（qdm 帧配置在哪里进行？直接配置driver，TRM不需要这些信息，只需要指导超帧内的总帧数（进入init）；超帧同步？秒脉冲给到了8710，如果配置了脉冲同步，start driver后，在秒脉冲时才启动（已经验证过））
...
#### `TRM_SendData`  （qdm A/B BRD/DEC 4种组合全支持；为什么要指定帧号发送？暂时不启用）
...
- `userId`: 用户ID，32位唯一标识符  （qdm：不需要，TRM根据规则按需要提取）
  - 范围: 0x00000000 - 0xFFFFFFFF （qdm：首位比特用于表示网络上下行）
...
#### `TRM_GetBeamInfo`  （仅用于未来定位技术开发，暂时不开放）
...
#### `TRM_SetCurrentFrame`  （qdm：进入init）
...
#### `TRM_SetMaxFrameCount`（qdm：进入init）
